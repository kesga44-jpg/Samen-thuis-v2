...
export async function cbsFuel(){const base='https://opendata.cbs.nl/ODataApi/OData/80416NED';const props=await getJson(`${base}/DataProperties`);const fields=(props.value||[]).filter(x=>/Benzine|Diesel|Lpg/i.test(x.Title||'')).map(x=>x.Key);const data=await getJson(`${base}/TypedDataSet?$orderby=Perioden%20desc&$top=1`);return{row:(data.value||[])[0]||null,fields,properties:props.value||[]};}
